    ...
    def cloud_segmentation_test(self, net, test_loader, config, num_votes=100, debug=False):
        ...
        if config.saving:
            ...
            if not exists(test_path):
                makedirs(test_path)
            if not exists(join(test_path, 'predictions','SoftwareNet/original_ply')):
                makedirs(join(test_path, 'predictions','SoftwareNet/original_ply'))
            if not exists(join(test_path, 'probs','SoftwareNet/original_ply')):
                makedirs(join(test_path, 'probs','SoftwareNet/original_ply'))
            if not exists(join(test_path, 'potentials','SoftwareNet/original_ply')):
                makedirs(join(test_path, 'potentials','SoftwareNet/original_ply'))
        else:
            test_path = None
    ...
    def slam_segmentation_test(self, net, test_loader, config, num_votes=100, debug=True):
        ...
        if test_loader.dataset.set == 'validation':
            ...
            for folder in ['predictions', 'probs']:
                if not exists(join(test_path, folder,'SoftwareNet/original_ply')):
                    makedirs(join(test_path, folder,'SoftwareNet/original_ply'))
